...
function App() {
    ...
    const toggleFormModalClickHandler = () : void => {
        setShowFormModal(!showFormModal);
    }

    const toggleDeleteConfirmationClickHandler = () : void => {
        setShowDeleteConfirmationModal(!showDeleteConfirmationModal);
    }

    useEffect(() => {
        for (const key in CivilStatus) {
            console.log(CivilStatus[key as keyof typeof CivilStatus]);
        }
    }, [])
            ...
            <Modal.Header>Form</Modal.Header>
                <Modal.Body>
                    ...
                    <div className="mb-2">
                        ...
                        <Select id="civilStatus" onChange={(event) => {console.log(event.target.value)}} defaultValue={"married"} required>
